...
import {
  Card, CardContent, Box, Typography, Chip, Stack, Button, TextField, MenuItem, Link as MuiLink,
} from '@mui/material'
...
export default function RecordsPage() {
  ...
  useEffect(() => {
    if (!patientId) return
    subscribeToPatient(patientId, () => qc.invalidateQueries({ queryKey: ['records', patientId] })).catch(() => {})
  }, [patientId, qc])
  ...
  return (
    <Box>
        ...
        <CardContent>
          ...
          {patientId && (
            <Stack direction="row" spacing={1} sx={{ mt: 2, alignItems: 'center' }}>
              ...
              <Button variant="contained" disabled={upload.isPending} onClick={() => {
      ...
      {patientId && (
        <Card>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 1.5 }}>Records</Typography>
            ...
            ) : <Typography variant="body2" color="text.secondary">No records for this patient.</Typography>}
          </CardContent>
        </Card>
      )}
    </Box>
