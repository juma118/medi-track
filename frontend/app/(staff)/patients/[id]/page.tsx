...
export default function PatientDetailPage() {
  ...
  return (
    <Box>
      <PageHeader
        title={p.fullName}
        subtitle={`DOB ${new Date(p.dateOfBirth).toLocaleDateString()} · ${p.bloodType ?? 'Unknown blood type'}`}
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card>
          <CardContent>
            ...
            <Row k="Blood type" v={p.bloodType} />
            <Row k="History" v={p.medicalHistory} />
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <CardContent>
            ...
            {appts.isLoading ? (
              ...
              <Stack spacing={1.5}>
                {appts.data.map((a) => (
                  <AppointmentRow key={a.id} a={a} doctor={isDoctor(role)} />
                ))}
              </Stack>
            ) : (
        ...
        <Card>
          <CardContent>
            ...
            {isDoctor(role) && appts.data && appts.data.length > 0 && (
              <PrescriptionForm appointmentId={appts.data[0].id} />
            )}
          </CardContent>
        </Card>
...
function AppointmentRow({ a, doctor }: { a: import('@/lib/types').Appointment; doctor: boolean }) {
  ...
  return (
    <Box sx={{ border: '1px solid', borderColor: 'divider', p: 1.5, borderRadius: 2 }}>
      ...
      <Typography variant="body2" color="text.secondary">
        Dr. {a.doctorName} · {a.reason ?? 'No reason'}
      </Typography>
      {a.diagnosis && (
    ...
    <Card>
      <CardContent>
        ...
        {thread.isLoading ? (
          ...
          <Thread messages={thread.data ?? []} mineIsPatient={false} />
        )}
        <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
          ...
          />
          <Button variant="contained" onClick={send} disabled={reply.isPending || !text.trim()}>
            Send
          </Button>
        </Stack>
      </CardContent>
  ...
  return (
    <Card>
      <CardContent>
        ...
        {create.isSuccess ? (
          <Alert severity="success">Portal account created. The patient can now sign in.</Alert>
        ) : (
          <Box
            component="form"
            onSubmit={(e: React.FormEvent) => {
