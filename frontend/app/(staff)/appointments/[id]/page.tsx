...
import {
  Card, CardContent, Box, Typography, Chip, Stack, Button, TextField,
} from '@mui/material'
import { useAppointment, useSetDiagnosis, useSetStatus, useCreatePrescription, usePatientPrescriptions } from '@/lib/hooks'
        ...
        <Card>
          <CardContent>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              <MuiPatientLink name={a.patientName} onClick={() => router.push(`/patients/${a.patientId}`)} />
              <Chip size="small" color={statusColor[a.status]} label={StatusName[a.status]} />
            </Box>
            <Typography variant="body2" color="text.secondary">Dr. {a.doctorName}</Typography>
            <Typography variant="body2" color="text.secondary">Reason: {a.reason ?? '—'}</Typography>
            ...
            <Box sx={{ mt: 2 }}>
              ...
              {doctor && (
                <Stack direction="row" spacing={1} sx={{ mt: 1 }}>
                  <TextField size="small" fullWidth placeholder="Record diagnosis" value={diag} onChange={(e) => setDiag(e.target.value)} />
...
function MuiPatientLink({ name, onClick }: { name: string; onClick: () => void }) {
  return <Typography onClick={onClick} sx={{ fontWeight: 700, color: 'primary.main', cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>{name}</Typography>
}

function AppointmentPrescriptions({ appointmentId, patientId }: { appointmentId: string; patientId: string }) {
  ...
  return (
    <Card>
      <CardContent>
        <Typography variant="h6" sx={{ mb: 1.5 }}>Prescriptions</Typography>
        {meds.data && meds.data.length > 0 && (
          <Stack spacing={1} sx={{ mb: 1.5 }}>
            {meds.data.map((m) => <Box key={m.id} sx={{ bgcolor: 'grey.50', p: 1, borderRadius: 2, fontSize: 14 }}>{m.medication} · {m.dosage} · {m.frequency}</Box>)}
          </Stack>
        )}
        <Box component="form" onSubmit={(e: React.FormEvent) => { e.preventDefault(); create.mutate({ appointmentId, ...f }, { onSuccess: () => setF({ medication: '', dosage: '', frequency: '' }) }) }}>
          <Stack spacing={1}>
