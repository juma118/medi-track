...
const tiles = [
  { key: 'totalPatients', label: 'Total Patients', icon: <PeopleAltRoundedIcon />, color: '#0d9488' },
  { key: 'todaysAppointments', label: "Today's Appointments", icon: <EventRoundedIcon />, color: '#0284c7' },
  { key: 'pendingAppointments', label: 'Pending', icon: <PendingActionsRoundedIcon />, color: '#d97706' },
  { key: 'activePrescriptions', label: 'Active Prescriptions', icon: <MedicationRoundedIcon />, color: '#4f46e5' },
  { key: 'newPatientsThisMonth', label: 'New This Month', icon: <PersonAddRoundedIcon />, color: '#059669' },
] as const
...
export default function DashboardPage() {
  ...
  const days = data.appointmentsLast7Days.map((d) => new Date(d.day).toLocaleDateString(undefined, { weekday: 'short' }))
    ...
    <Box>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        {tiles.map((t) => (
          <Card key={t.key}>
            <CardContent>
              <Avatar variant="rounded" sx={{ bgcolor: `${t.color}1a`, color: t.color, mb: 1.5 }}>{t.icon}</Avatar>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardContent>
            <Typography variant="h6" sx={{ mb: 2 }}>Appointments — last 7 days</Typography>
            <BarChart
              height={280}
              ...
              <Stack spacing={1.5}>
                {today.data.map((a) => (
                  <Box key={a.id} onClick={() => router.push(`/appointments/${a.id}`)}
                    sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', p: 1, borderRadius: 2, '&:hover': { bgcolor: 'action.hover' } }}>
                    <Box>
                      <Typography sx={{ fontWeight: 600, fontSize: 14 }}>{a.patientName}</Typography>
                      <Typography variant="caption" color="text.secondary">
                        {new Date(a.scheduledAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · {a.doctorName}
                      </Typography>
                    </Box>
