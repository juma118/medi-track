...
import { Card, CardContent, Box, Typography, Stack, Button, TextField, MenuItem } from '@mui/material'
import { usePatients, usePatientPrescriptions, usePatientAppointments, useCreatePrescription } from '@/lib/hooks'
...
export default function PrescriptionsPage() {
  ...
  return (
    <Box>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <TextField select label="Patient" value={patientId} onChange={(e) => setPatientId(e.target.value)} sx={{ minWidth: 280 }}>
      ...
      {patientId && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 1.5 }}>Add prescription</Typography>
              {appointmentId ? (
                <Box component="form" onSubmit={(e: React.FormEvent) => { e.preventDefault(); create.mutate({ appointmentId, ...f, expiryDate: f.expiryDate || undefined }, { onSuccess: () => setF({ medication: '', dosage: '', frequency: '', expiryDate: '' }) }) }}>
                  <Stack spacing={1.5}>
                    ...
                    <TextField size="small" type="date" label="Expiry" slotProps={{ inputLabel: { shrink: true } }} value={f.expiryDate} onChange={(e) => set('expiryDate', e.target.value)} />
                    <Button type="submit" variant="contained" disabled={create.isPending}>Add prescription</Button>
                    <Typography variant="caption" color="text.secondary">Linked to the patient&apos;s most recent appointment.</Typography>
                  </Stack>
                </Box>
              ) : <Typography variant="body2" color="text.secondary">This patient has no appointment to attach a prescription to.</Typography>}
            </CardContent>
          </Card>
        </div>
      )}
