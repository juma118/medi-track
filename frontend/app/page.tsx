...
const features = [
  {
    ...
    title: 'AI assistant',
    desc: 'Symptom analysis, document summarization, and grounded patient-history chat.',
  },
  {
    icon: <BoltRoundedIcon />,
    title: 'Event-driven',
    desc: 'Kafka-backed workers handle AI, notifications, and audit asynchronously.',
  },
  {
    icon: <InsightsRoundedIcon />,
    title: 'Live dashboard',
    desc: 'Redis-cached KPIs and real-time updates over SignalR.',
  },
  {
    icon: <ShieldRoundedIcon />,
    title: 'Role-based access',
    desc: 'Scoped, audited access for doctors, receptionists, and patients.',
  },
]
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
      <AppBar
        ...
        sx={{
          ...
          color: 'white',
          boxShadow: '0 2px 14px rgba(13,148,136,0.3)',
        }}
      >
        <Container maxWidth="lg">
          <Toolbar disableGutters sx={{ gap: 1.25 }}>
            <Avatar sx={{ bgcolor: 'white', color: 'primary.dark', width: 36, height: 36 }}>
              <LocalHospitalRoundedIcon fontSize="small" />
            </Avatar>
            <Typography
              variant="h6"
              sx={{ fontWeight: 800, flexGrow: 1, color: 'white', letterSpacing: '-0.01em' }}
            >
              MediTrack
            </Typography>
            {loggedIn ? (
      ...
      <Box
        sx={{
          position: 'relative',
          overflow: 'hidden',
          background: 'linear-gradient(155deg, #0f766e 0%, #0d9488 45%, #f59e0b 165%)',
          color: 'white',
        }}
      >
        <Box
          sx={{
            ...
            opacity: 0.18,
            backgroundImage:
              'radial-gradient(circle at 15% 15%, #fff 0, transparent 38%), radial-gradient(circle at 90% 10%, #fde68a 0, transparent 30%)',
          }}
        />
        <Container maxWidth="lg" sx={{ position: 'relative', py: { xs: 7, md: 12 } }}>
          <div className="grid items-center gap-10 md:grid-cols-2">
            <Box>
              <Chip
                label="AI Powered · HealthTech Platform"
                sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white', mb: 3, fontWeight: 600 }}
              />
              <Typography
                variant="h2"
                sx={{ fontSize: { xs: 36, md: 56 }, lineHeight: 1.06, mb: 2 }}
              >
                Caring for patients, doctors &amp; clinics — together.
              </Typography>
              <Typography
                sx={{ fontSize: { xs: 16, md: 19 }, opacity: 0.94, mb: 4, maxWidth: 560 }}
              >
                Manage patients, appointments, records and prescriptions — with an AI assistant and
                a warm, self-service patient portal.
              </Typography>
            ...
            <Box sx={{ position: 'relative', display: { xs: 'none', md: 'block' } }}>
              ...
              >
                <Avatar variant="rounded" sx={{ bgcolor: 'success.main', width: 38, height: 38 }}>
                  <SmartToyRoundedIcon />
                </Avatar>
                <Box>
                  <Typography sx={{ fontWeight: 800, color: 'text.primary', lineHeight: 1 }}>
                    AI summaries
                  </Typography>
          ...
          {features.map((f) => (
            <Card key={f.title} sx={{ height: '100%' }}>
              <CardContent>
                <Avatar
                  variant="rounded"
                  sx={{ bgcolor: 'primary.light', color: 'primary.dark', mb: 1.5 }}
                >
                  {f.icon}
                </Avatar>
        ...
        <Container maxWidth="lg" sx={{ py: { xs: 7, md: 10 } }}>
          <Box sx={{ textAlign: 'center', mb: 6 }}>
            ...
            <Typography color="text.secondary" sx={{ maxWidth: 560, mx: 'auto' }}>
              A tailored experience per role — patients get self-service, staff get the tools they
              need.
            </Typography>
          </Box>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {roles.map((r) => (
              <Card key={r.title} sx={{ height: '100%', position: 'relative', overflow: 'hidden' }}>
                ...
                <CardContent sx={{ p: 3 }}>
                  <Avatar variant="rounded" sx={{ bgcolor: r.color, mb: 2 }}>
                    {r.icon}
                  </Avatar>
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 7, md: 10 } }}>
        ...
        >
          ...
          <Typography sx={{ opacity: 0.8, mb: 3 }}>
            Sign in with a demo account to explore the full platform.
          </Typography>
          <Button
            size="large"
            variant="contained"
            onClick={() =>
              router.push(loggedIn ? (isPatient(role) ? '/portal' : '/dashboard') : '/login')
            }
          >
